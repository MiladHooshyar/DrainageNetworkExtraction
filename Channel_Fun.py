...
import numpy as np
from numpy import arange, cos, linspace, pi, sin, random , array
...
##########################################################################
def channel_head_find(por_file_path , map_file_path, text_file_path , flow_acc_raster , flow_dir_raster, curve_Raster , dem_raster ,  number_of_uni_point , number_contour , method, flage_plot):
...
##            ax.annotate(int(p) , xy=(por_point[p][0], por_point[p][1]))
        ...
        if number_of_ID <= 5:
            ...
            for ID in range (0,  number_of_ID):
                temp_kapa_1 , x_0 , y_0 ,  x_p , y_p , x , y = poly_fit(ID, 'basin_' + str(p) +'_point' , por_file_path , por_file_path , number_of_uni_point , 0 , x_0 , y_0)
            ...
            org_x , org_y , ID_elevation, number_of_ID = contour_delete(sorted_x , sorted_y , sorted_ID_elevation, number_of_ID, number_of_uni_point , contour_interval , por_point[p][2] , por_point[p][3])
        ...
        if num_curve <= 5:
            ...
            for initial_tresh in range (0 , number_of_ID):
                ...
                if method == 'contour_cluster':
                    x_ave , y_ave = cluster_average(initial_cluster_code , x_data , y_data , number_of_k , number_of_ID, number_of_uni_point)
                    err = cluster_performance(x_data , y_data ,initial_cluster_code , x_ave , y_ave , number_of_k , number_of_ID , number_of_uni_point)
...
##                    count = count + (lenght_error[i , 0] / lenght_error[i , 1]) ** 2
                    ele_tresh = ele_tresh + ID_elevation[i , 1] * lenght_error[i , 0] 
...
def find_local_min(ele_error):
    ...
    lenght_error = np.zeros((num_p , 2))
    count = 1
    for i in range (0 , num_p - 1):
        if ele_error[i + 1 , 1] >= ele_error[i , 1]:
            if count > 1:
                lenght_error[i , :] = [count , ele_error[i , 1]]
            count = 1
        else:
            count = count + 1
    count = 0
    for i in range (num_p - 1 , 0 , -1):
        if ele_error[i - 1 , 1] >= ele_error[i , 1]:
            lenght_error[i , 0] =  lenght_error[i , 0] + count
            count = 0
        else:
            count = count + 1
...
####################READING#####################################
    with open('%s' %in_file_path +'/' + '%s' %name + '_'+ str(int(ID)) + '.txt') as f:
        ...
        for line in f:
            ...
            if i > 0:
                temp_t.append(temp_t[i - 1] + np.sqrt((temp_x[i] - temp_x[i - 1]) ** 2 + (temp_y[i] - temp_y[i - 1]) ** 2))
            else:
                temp_t.append(0)
            i = i + 1
    f.close()
...
################################################################
    ...
    for i in range(1 , number_of_uni_point - 1):
        ...
        kapa[i] = (dre_x_t[i] * dre_2_y_t[i] - dre_y_t[i] * dre_2_x_t[i]) / (dre_x_t[i] ** 2 + dre_y_t[i] ** 2) ** (3 / 2)
...
def curvature(x , y , number_of_uni_point):
    ...
    for i in range(1 , number_of_uni_point - 1):
        ...
        kapa[0 , i] = (dre_x_t[0 , i] * dre_2_y_t[0 , i] - dre_y_t[0 , i] * dre_2_x_t[0 , i]) / (dre_x_t[0 , i] ** 2 + dre_y_t[0 , i] ** 2) ** (3 / 2)
...
##    f.close()
##
##    plot_contour = np.array(contour)
##    x = plot_contour[:,2]
##    y = plot_contour[:,3]
##
##    # ploting
##    pl.plot(x,y,'g-',linewidth=0.1)
...
def whole_to_part(kapa_1, kapa_2):
    ...
    if number_of_point_2 >= number_of_point_1:
        ...
        for u in range (0 , max_u):
            ...
            denume_2 = 0
            for i in range (0 , number_of_point_1):
                nume = nume + (kapa_1[i] - ave_1) * (kapa_2[i + u] - ave_2)
                denume_1 = denume_1 + (kapa_1[i] - ave_1) ** 2
                denume_2 = denume_2 + (kapa_2[i + u] - ave_2) ** 2
            if (denume_1 * denume_2) == 0:
                v[u] =  -5
            else:
                v[u] = nume / (denume_1 * denume_2) ** 0.5 
...
def contour_sym_curve(x , y , org_x , org_y, curve_Array , basin_Array, ridge_Array, number_of_point , number_of_ID , X_min , Y_max , dx , dy):
        ...
        for i in range (mid_i[0 , ID] , number_of_point - 3):
            ...
            for next_i in range (i + 1 , i + 3):
                ...
                if ridge_Array[next_ii , next_jj] == 1:
                    flage_end_ridge = 0
            if ridge_Array[ii , jj] == 1 and flage_end_ridge == 1:
                upper_bond = i
                break
...
def contour_direction_fix(X , Y , ID_elevation, number_of_ID, number_of_uni_point):
    ...
    for ID in range (0 , number_of_ID - 1):
        ...
        V_3 = [(X[ID , number_of_uni_point - 1] - X[next_ID , mid_index]) , (Y[ID , number_of_uni_point - 1] - Y[next_ID , mid_index])]

##        if ID == 1:
##            pl.plot([X[next_ID , mid_index] ,  X[ID , mid_index]] , [Y[next_ID , mid_index] ,  Y[ID , mid_index]])
##            pl.plot([X[ID , mid_index - 1] ,  X[ID , mid_index]] , [Y[ID , mid_index - 1] , Y[ID , mid_index]])
            

        #print ID , ID_elevation[ID , 1] , np.cross(V_1 , V_2)

        #print np.cross(V_1 , V_2) , np.cross(V_1 , V_3)
...
##        print X[ID,:]
##        print Y[ID,:]
        Teta = math.atan2((org_Y[ID , number_of_uni_point - 1] - org_Y[ID , 0]) , (org_X[ID , number_of_uni_point - 1] - org_X[ID , 0]))
        for i in range (0 , number_of_uni_point):
   ...
   ##############################################################
    ...
    for ID in range (0 , new_0_number_of_ID):
        for i in range (0 , number_of_uni_point - 1):
            contour_len[ID][0] = contour_len[ID][0] + ((new_0_X[ID][i] - new_0_X[ID][i + 1]) ** 2 + (new_0_Y[ID][i] - new_0_Y[ID][i + 1]) ** 2) ** 0.5
        ...
        for ID_1 in range (0 , new_0_number_of_ID):
            ...
            for ID_2 in range (ID_1 + 1 , new_0_number_of_ID):
                if deleted_code[0][ID_2] == 0 and deleted_code[0][ID_1] == 0:
                    if contour_len[ID_1][0] <  0.8 * contour_len[ID_2][0] or new_0_ID_elevation[ID_1][0] == new_0_ID_elevation[ID_2][0] or contour_len[ID_1] < 0.1 * average_len:
                        deleted_code[0][ID_1] = 1
                        num_delet = num_delet + 1
                    break 
    sum_deleted_code = 0
...
##    for ID_1 in range (0 , new_0_number_of_ID):
##        for ID_2 in range (0 , new_0_number_of_ID):
##            if new_0_ID_elevation[ID_1][1] == new_0_ID_elevation[ID_2][1] and ID_1 != ID_2:
##                if  contour_len[ID_1][0] <  contour_len[ID_2][0]:
##                    deleted_code[0][ID_1] = 1
##                    deleted_code[0][ID_2] = 1
##                elif contour_len[ID_1][0] >  contour_len[ID_2][0]:
##                    deleted_code[0][ID_2] = 1
##                    deleted_code[0][ID_1] = 1
##
##    sum_deleted_code = 0
##    for ID in range (0 , new_1_number_of_ID):
##        sum_deleted_code = sum_deleted_code + deleted_code[0][ID]
##
##    new_1_number_of_ID = int(new_0_number_of_ID - sum_deleted_code)
...
def cluster_assign(x_data , y_data , x_ave , y_ave , number_of_k , number_of_ID , number_of_uni_point , old_cluster_code , min_number_contour):
    ...
    for ID in range (1 , number_of_ID - 1):
        ...
        if count_code[0][old_k] > min_number_contour[old_k]:
            if old_cluster_code[0][ID] != old_cluster_code[0][ID + 1] or old_cluster_code[0][ID] != old_cluster_code[0][ID - 1] and flage_change == 0:
                min_diff = 10 ** 10
...
def cluster_find(x_data , y_data , number_of_k , number_of_ID, number_of_uni_point , min_contour , initial_cluster_code):
    ...
    #print cluster_code , elevation_bond
    ...
    while flage == 1:
        ...
        cluster_code = cluster_assign(x_data , y_data , x_ave , y_ave , number_of_k , number_of_ID , number_of_uni_point, old_cluster_code , min_number_contour)
