...
def neg_bond(curvature_file, valley_bond_file, ridge_bond_file, convergent_sig_file, divergent_sig_file):
    ...
    perc90_valley = ndimage.labeled_comprehension(curve_Array, Lab_Val_Array, np.arange(1, num_label_val + 1),
                                                  percentile, float, 0)
    perc90_ridge = ndimage.labeled_comprehension(-1 * curve_Array, Lab_Rid_Array, np.arange(1, num_label_rid + 1),
                                                 percentile, float, 0)

    area_valley = ndimage.labeled_comprehension(valley, Lab_Val_Array, np.arange(1, num_label_val + 1), np.sum, float,
                                                0)
    ...
    arcpy.NumPyArrayToRaster(perc90_valley, corner, dx, dx, value_to_nodata=0).save(convergent_sig_file)
...
def Perona_malik_iter(dem_Array, num_iter, resolution, option):
    ...
    for it in range(0, num_iter):
        ...
        # Calculate derivatives in for directions
        delta_n = (np.append(dem_Array[0, :].reshape(1, Nj), dem_Array[0: Ni - 1, :].reshape(Ni - 1, Nj),
                             axis=0) - dem_Array) / resolution
        delta_s = (np.append(dem_Array[1: Ni, :].reshape(Ni - 1, Nj), dem_Array[Ni - 1, :].reshape(1, Nj),
                             axis=0) - dem_Array) / resolution
        delta_e = (np.append(dem_Array[:, 1: Nj].reshape(Ni, Nj - 1), dem_Array[:, Nj - 1].reshape(Ni, 1),
                             axis=1) - dem_Array) / resolution
        delta_w = (np.append(dem_Array[:, 0].reshape(Ni, 1), dem_Array[:, 0: Nj - 1].reshape(Ni, Nj - 1),
                             axis=1) - dem_Array) / resolution
        ...
        if 0.01 > 0:
            delta_n = (np.append(org_dem_Array[0, :].reshape(1, Nj), org_dem_Array[0: Ni - 1, :].reshape(Ni - 1, Nj),
                                 axis=0) - org_dem_Array) / resolution
            delta_s = (np.append(org_dem_Array[1: Ni, :].reshape(Ni - 1, Nj), org_dem_Array[Ni - 1, :].reshape(1, Nj),
                                 axis=0) - org_dem_Array) / resolution
            delta_e = (np.append(org_dem_Array[:, 1: Nj].reshape(Ni, Nj - 1), org_dem_Array[:, Nj - 1].reshape(Ni, 1),
                                 axis=1) - org_dem_Array) / resolution
            delta_w = (np.append(org_dem_Array[:, 0].reshape(Ni, 1), org_dem_Array[:, 0: Nj - 1].reshape(Ni, Nj - 1),
                                 axis=1) - org_dem_Array) / resolution
            delta_n = np.nan_to_num(delta_n)
            delta_s = np.nan_to_num(delta_s)
            delta_e = np.nan_to_num(delta_e)
        ...
        dem_Array = dem_Array + 0.05 * (
                np.multiply(Cn, delta_n) + np.multiply(Cs, delta_s) + np.multiply(Ce, delta_e) + np.multiply(Cw,
                                                                                                             delta_w))
...
def curvature(DEM_file, plan_curvature_file, profile_curvature_file, resolution, option):
    # Option:
    # POLY_CON: polygon fitting contour curvature
    # POLY_LAP: polygon fitting laplacian
    ...
    if option == 'POLY_CON' or option == 'POLY_LAP':
        ...
        if option == 'POLY_CON':
            curve_Array_plan = (hxx * hy ** 2 - 2 * hxy * hx * hy + hyy * hx ** 2) / (
                    (hx ** 2 + hy ** 2) * (1 + hx ** 2 + hy ** 2) ** 0.5)
        ...
        if option == 'DIFF_LAP':
            ...
            p = (grad_x ** 2 + grad_y ** 2)
            q = 1 + p
            curve_Array = (grad_xx * grad_y ** 2 - 2 * grad_xy * grad_x * grad_y + grad_yy * grad_x ** 2) / (
                    p * q ** 0.5)
...
def stream_to_line(in_file_path, out_file_path, stream_raster, flow_dir_raster, out_polyline):
    ...
    StreamToFeature(stream_raster_1, '%s' % in_file_path + '/' + '%s' % flow_dir_raster,
                    '%s' % in_file_path + '/' + '%s' % out_polyline)


def stream_smooth(in_file_path, out_file_path, in_polyline, out_polyline, tolerance):
    in_polyline = '%s' % in_file_path + '/' + '%s' % in_polyline
    out_polyline = '%s' % out_file_path + '/' + '%s' % out_polyline
    arcpy.cartography.SmoothLine(in_polyline, out_polyline, "PAEK", tolerance)
...
def flow_acc_weight(Dir_raster, Wei_raster, Acc_raster, option):
    if option == 'D8':
        dataType = "FLOAT"
...
def CC_dis(in_file_path, curve_raster, ave_k, figure_name):
    ...
    cumfreqs, lowlim, binsize, extrapoints = scipy.stats.cumfreq(curve_Array, numbins=num_bins,
                                                                 defaultreallimits=(min_curve, max_curve))
...
def Dinf_fix(Dinf_raster, Curve_raster, Fix_Dinf_raster):
    ...
    i_1, j_1 = move_cal(dir_1, np.arange(0, number_row).reshape(number_row, 1),
                        np.arange(0, number_col).reshape(1, number_col), 0, number_row, 0, number_col)
    i_2, j_2 = move_cal(dir_2, np.arange(0, number_row).reshape(number_row, 1),
                        np.arange(0, number_col).reshape(1, number_col), 0, number_row, 0, number_col)
...
def find_end(in_stream_Array, heads_list, dir_Array):
    ...
    for point in heads_list:
        ...
        while flag_end == 0:
            ...
            if bond_check(new_i, 0, num_row) == 0 or bond_check(new_j, 0, num_col) == 0 or bond_check(move, 1,
                                                                                                      129) == 0 or bond_check(
                dir_Array[new_i, new_j], 1, 129) == 0:
                flag_end = 1
...
def connect_stream_smart(in_file_path, out_file_path, stream_raster, flow_dir_raster, curve_raster, out_raster,
                         connect_ratio):
    total_num_add = 0
    ...
    for index in heads_list:
        p = p + 1
        list_passed, last_point, length, length_curve, visited = move_forward_connect_1(index, stream_Array, dir_Array,
                                                                                        visited, curve_Array, num_row,
                                                                                        num_col)
        ...
        num_end[last_point] = num_end[last_point] + length_curve
    num_add = 1
    while num_add > 0:
        num_add = 0
        for index in list_end:
            list_passed_gap, last_point_gap, length_gap, length_curve_gap = move_forward_gap(index, stream_Array,
                                                                                             dir_Array, curve_Array,
                                                                                             num_row, num_col)
            ...
            if length_gap > 0 and length_gap <= max_gap:  # and length > 2:
                ...
                num_end[last_point] = num_end[last_point] - float(length_gap) / connect_ratio + num_end[
                    index]  # + length_curve_gap
                list_end.remove(index)

    arcpy.NumPyArrayToRaster(stream_Array, corner, dx, dx, value_to_nodata=0).save(
        '%s' % out_file_path + '/' + '%s' % out_raster)
...
def move_forward_gap(index, Sp_stream_Array, dir_Array, curve_array, number_row, number_col):
    ...
    while flag_end == 0:
        ...
        if bond_check(new_i, 0, number_row) == 0 or bond_check(new_j, 0, number_col) == 0 or bond_check(move, 1,
                                                                                                        129) == 0 or bond_check(
            dir_Array[new_i, new_j], 1, 129) == 0:
            # last_point = (new_i , new_j)
            flag_end = 1
...
def delete_isolated_stream(in_file_path, out_file_path, stream_raster, order_raster, flow_dir_raster, acc_raster,
                           min_length_iso, min_length_con, out_raster):
    num_deleted = 0
    ...
    for index in heads_list:
        list_passed, last_point, flag_connected_end, ave_curve, length = move_forward(index, Sp_stream_Array,
                                                                                      dir_Array, dir_Array,
                                                                                      order_Array, number_row,
                                                                                      number_col)
        if flag_connected_end == 0 and length <= min_length_iso:
            for point in list_passed:
                stream_Array[point] = 0
                num_deleted = num_deleted + 1
        if flag_connected_end == 1 and length <= min_length_con:
            for point in list_passed:
                stream_Array[point] = 0
                num_deleted = num_deleted + 1
...
def move_backward(dir_Array, Sp_stream_Array, ele_Array, order_Array, list_passed, last_point, number_row, number_col,
                  ele_thresh, length_thresh, head):
    alt_stream_list = [[]]
    # num_alt_st = 1
    alt_st = 0
    list_passed_new = []
    ...
    while flag_done == 0:
        ...
        while flag_out == 0:
                    ...
                    if bond_check(new_i, 0, number_row) == 1 and bond_check(new_j, 0, number_col) == 1:
                        ...
                        if bond_check(move, 1, 129) == 1:
                            ...
                            if Sp_stream_Array[new_i, new_j] == 1 and (next_i, next_j) == position and \
                                    (any((p == (new_i, new_j)) for p in list_passed)) == False and (
                                    any((p == (new_i, new_j)) for p in list_passed_new)) == False:
            ...
            ##            if head == 138:
            ##                    print count_found , temp_ele_thresh , ele_Array[next_position]

            if count_found > 0 and ele_Array[position] <= temp_ele_thresh and length <= temp_length_thresh:
...
def move_forward(index, Sp_stream_Array, dir_Array, curve_Array, order_Array, number_row, number_col):
    ...
    while flag_end == 0:
        ...
        if bond_check(new_i, 0, number_row) == 0 or bond_check(new_j, 0, number_col) == 0 or bond_check(move, 1,
            ...
            last_point = (new_i, new_j)
            flag_end = 1
            flag_connected_end = 1
        else:
            last_point = (new_i, new_j)
            length = length + 1

    return list_passed, last_point, flag_connected_end, (ave_curve / length), length


########################################################################################
########################################################################################
def stream_delete_small_fast(in_file_path, out_file_path, ridge_raster, stream_raster, dem_raster, flow_dir_raster,
                             acc_raster, order_raster, curve_raster, out_raster):
    num_deleted = 0
    ...
    for index in heads_list:
        ...
        if stream_Array[index] == 1:
            flag_delete = 0
            list_passed, last_point, flag_connected_end, _, length = move_forward(index, Sp_stream_Array, dir_Array,
                                                                                  curve_Array, order_Array, number_row,
                                                                                  number_col)
            list_passed.reverse()
            if flag_connected_end == 1:
                alt_stream_list, num_alt_st, alt_length_list = move_backward(dir_Array, Sp_stream_Array, ele_Array,
                                                                             order_Array, list_passed, last_point,
                                                                             number_row, number_col,
                                                                             (ele_Array[index] + 20), (1.25 * length),
                                                                             p)
                    ...
                    if flag_delete == 0:
                        ...
                        for point in list_passed:
                            if pp < len(alt_stream_list[alt_st]):
                                ...
                                pp = pp + 1
                        flag_delete_stream = 0
                        flag_delete_alt_stream = 0
                        ...
                        if len(list_same_1) > 0 and len(list_same_2) > 0:
                            ...
                            # they are both first order
                                ...
                                if length <= 0.5 * alt_length_list[alt_st]:
                                    ...
                                    flag_delete_alt_stream = 1
                                # they are in same size
                                elif win_point < 0.5 * min(len(list_same_1), len(list_same_2)):
                                    flag_delete_stream = 1
                                elif win_point == 0.5 * min(len(list_same_1),
                                                            len(list_same_2)) and sum_curve_2 > sum_curve_1:
                                    flag_delete_stream = 1
                                else:
                                    flag_delete_alt_stream = 1
                        if flag_delete_stream == 1:
                            flag_delete = 1
                            if flag_ridge_found == 1:
...
def stream_extention(in_file_path, out_file_path, stream_raster, old_stream_raster, dem_raster, flow_dir_raster,
                     acc_raster, order_raster, old_order_raster, curve_raster, out_raster):
    DIR_RASTER = '%s' % in_file_path + '/' + '%s' % flow_dir_raster
    ACC_RASTER = '%s' % in_file_path + '/' + '%s' % acc_raster
    DEM_RASTER = '%s' % in_file_path + '/' + '%s' % dem_raster
    ORDER_RASTER = '%s' % in_file_path + '/' + '%s' % order_raster
    ...
    for index in heads_list:
        list_passed, last_point, flag_connected_end, ave_curve, length = move_forward(index, Sp_stream_Array,
                                                                                      ...
                                                                                      number_col)
        if flag_connected_end == 1:
            alt_stream_list, num_alt_st, alt_length_list = move_backward(dir_Array, old_Sp_stream_Array, ele_Array,
                                                                         old_order_Array, list_passed, index,
                                                                         number_row, number_col, (ele_Array[index] + 1),
                                                                         (1.25 * length), 0)
            if num_alt_st > 0:
                max_curve = 0
                for alt_st in range(0, num_alt_st):
...
def find_por_point(in_file_path, out_file_path, stream_raster, dem_raster, flow_dir_raster, acc_raster, order_raster,
                   out_text):
    DIR_RASTER = '%s' % in_file_path + '/' + '%s' % flow_dir_raster
    ORDER_RASTER = '%s' % in_file_path + '/' + '%s' % order_raster
    DEM_RASTER = '%s' % in_file_path + '/' + '%s' % dem_raster
...
def unchannel_delete(in_file_path, out_file_path, stream_raster, flow_dir_raster, order_raster, Chan_Unchan,
                     Unch_thresh,
                     out_raster):
    DIR_RASTER = '%s' % in_file_path + '/' + '%s' % flow_dir_raster
    ORDER_RASTER = '%s' % in_file_path + '/' + '%s' % order_raster
    ...
    cleaned_st_raster = arcpy.NumPyArrayToRaster(stream_Array, corner, dsc.meanCellWidth, dsc.meanCellHeight,
                                                 value_to_nodata=0)
...
def head_delete(in_file_path, out_file_path, dem_raster, stream_raster, flow_dir_raster, order_raster, head_ele,
                out_raster):
